#!/usr/bin/env python3
"""
generate_hacking_tools_docs.py

Creates a folder ("Security_Tools_Reference") and, inside it, one text file
per well-known security / penetration-testing tool. Each file contains:
...
configure any tool -- it just writes reference notes to disk, all sourced
from each tool's own public, well-known usage patterns.
...
def safe_filename(name: str) -> str:
    """Convert a tool name into a safe filename."""
    return name.lower().replace(" ", "_").replace("/", "_") + ".txt"


def build_file_content(name: str, info: dict) -> str:
    lines = []
    lines.append("=" * 70)
    ...
    lines.append("")
    lines.append(f"CATEGORY / USED FOR:\n  {info['category']}")
    lines.append("")
    ...
    for step in info["install"]:
        lines.append(f"  - {step}")
    lines.append("")
    lines.append("BASIC / COMMON COMMANDS:")
    for cmd in info["basic_commands"]:
        lines.append(f"  {cmd}")
    lines.append("")
    lines.append("-" * 70)
...
def build_index_content(tools: dict) -> str:
    lines = []
    lines.append("=" * 70)
    lines.append("SECURITY TOOLS REFERENCE - INDEX")
    lines.append("=" * 70)
    lines.append("")
    lines.append(DISCLAIMER)
    lines.append("")
    lines.append(f"Total tools documented: {len(tools)}")
    lines.append("")
    for name, info in tools.items():
        lines.append(f"- {name:<20} -> {info['category']}   (file: {safe_filename(name)})")
    lines.append("")
    lines.append(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    return "\n".join(lines)
...
def main():
    ...
    try:
        ...
        print(f"Failed to create folder '{base_dir}': {e}")
        sys.exit(1)

    print(f"Creating reference folder: {base_dir}\n")

    # Write index file
    index_path = os.path.join(base_dir, "00_INDEX.txt")
    with open(index_path, "w", encoding="utf-8") as f:
        f.write(build_index_content(TOOLS))
    print(f"  [+] Created {os.path.basename(index_path)}")

    # Write one file per tool
    for name, info in TOOLS.items():
        filename = safe_filename(name)
        filepath = os.path.join(base_dir, filename)
        content = build_file_content(name, info)
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(content)
        print(f"  [+] Created {filename}")

    print(f"\nDone. {len(TOOLS)} tool files + 1 index file created in '{base_dir}'.")
